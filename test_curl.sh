...
#curl -s "http://localhost:5000/api/get_question?shiken_kubun=AP&shiken_day=2023/04/16&day_kubun=${day_kubun}&que_no=1" | jq .


day_kubun=$(echo "午前" | nkf -WMQ | tr = %)
curl -s "http://localhost:5000/api/get_questions?shiken_kubun=AP&shiken_day=2023/04/16&day_kubun=${day_kubun}" | jq .
